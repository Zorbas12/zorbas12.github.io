...
async function registerDeviceOnce() {
  ...
  try {
    ...
    if (!snap.exists()) {
      await set(deviceRef, { ...base, ip: ip || "Unknown", role: "user", nickname: info.name, timestamp: now });
    } else {
      const existing = snap.val();
      // Keep nickname in sync unless it was renamed manually.
      if (!existing.nickname || existing.nickname === existing.name || existing.nickname === existing.model) {
        base.nickname = info.name;
      }
      // Drop the legacy browser field — device identity no longer uses it.
      if (existing.browser !== undefined) base.browser = null;
      await update(deviceRef, base);
    }
  } catch {}
}
...
export function triggerNotification(targetDeviceId, title, body) {

  // Clean title — remove emojis from title, keep in body
  ...
  push(ref(db, `notifications/${targetDeviceId}`), {
    title: cleanTitle, body: cleanBody, timestamp: Date.now(), delivered: false
  });

  // Channel 2: OneSignal push (for Android app, even when closed)
  // Key is stored server-side in Vercel env — no client key needed
